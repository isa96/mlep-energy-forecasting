...
from ETL import utils, settings
...
def create(
    ...
    project = hopsworks.login(
        api_key_value=settings.SETTING["FS_API_KEY"], project="energy_consumption_mlep"
    )
    ...
    try:
        name_feature_views = "energy_consumption_denmark_view"
        feature_views = fs.get_feature_views(name=name_feature_views)
    except hsfs.client.exceptions.RestAPIError:
        logger.info(f"No features views found for {name_feature_views}")
        feature_views = []
    ...
    # create feature view given feature group version
    ...
    feature_view = fs.create_feature_view(
        ...
        description="Energy consumption denmark, this project forecasting energy.",
        query=ds_query,
        labels=[]
    )
    ...
    feature_view.create_training_data(
        description="Energy training dataset",
        data_format="csv",
